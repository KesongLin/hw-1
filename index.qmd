...
```
From my_vec_bool we can easily see that there are 4 false, so there are 4 elements greater than 0
```
...
matrix2.1 <- matrix(numbers, nrow = 1, ncol = 100, byrow = TRUE )
matrix2.2 <- matrix(squared_numbers, nrow = 1, ncol = 100, byrow = TRUE )
matrix2 <- rbind(matrix2.1, matrix2.2)
...
```
I think the difference in performance between the two functions should negligible, because they just count in different way.
But if a specific matrix is available, the performance between the two functions could be different
```
...
```
col wise scan took longer to run
...
```
it seems like row_wise_scan will took longer time than col_wise_scan
